...
int main(int argc, char **argv) {
  ...
  // Realistic dim-3 tensor chain shapes, including odd post sizes used by serial backends
  for (CeedInt P = 2; P <= 10; P++) {
    for (CeedInt Q = P - 1; Q <= P + 2; Q++) {
      if (Q < 1) continue;
      const TensorCase chains[6] = {
          {P * P, P, 1, Q}, {P, P, Q, Q}, {1, P, Q * Q, Q},  // forward chain contractions
          {Q * Q, Q, 1, P}, {Q, Q, P, P}, {1, Q, P * P, P},  // transpose chain contractions
      };

      for (size_t i = 0; i < sizeof(chains) / sizeof(chains[0]); i++) {
        for (CeedTransposeMode t_mode = CEED_NOTRANSPOSE; t_mode <= CEED_TRANSPOSE; t_mode++) {
          for (CeedInt add = 0; add <= 1; add++) {
            if (RunCase(contract, chains[i], t_mode, add)) return 1;
          }
        }
      }
    }
